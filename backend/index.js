...
async function initDB() {
  try {
    pool = mysql.createPool({
      ...
      database: process.env.MYSQL_DATABASE || 'xiaoxiao_diary',
...
app.get('/api/diaries', async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT * FROM diaries ORDER BY created_at DESC');
...
app.get('/api/diaries/:id', async (req, res) => {
  try {
    const [rows] = await pool.execute('SELECT * FROM diaries WHERE id = ?', [req.params.id]);
    if (rows.length === 0) {
...
app.post('/api/diaries', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: { id: result.insertId, title, content, weather, mood } });
  } catch (error) {
...
app.delete('/api/diaries/:id', async (req, res) => {
  try {
    await pool.execute('DELETE FROM diaries WHERE id = ?', [req.params.id]);
